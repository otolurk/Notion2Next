...
/**
 * Endspace Target Cursor
 * 移植自 blog-cursor/TargetCursor.jsx
 * 旋转的准星光标，悬停在可交互元素上时四角展开吸附到元素边界
 *
 * 注意：position:fixed 元素在有 transform/perspective/filter 的祖先下会
 * 以该祖先为包含块，导致坐标错位。这里测量并补偿偏移量。
 */
...
// 隐藏系统光标（配合 style.js 中的 body.endspace-target-cursor-on 规则）
const CURSOR_BODY_CLASS = 'endspace-target-cursor-on'

const TargetCursor = ({
  targetSelector = 'a, button, [role="button"], .ef-btn, .cursor-target',
  spinDuration = 2,
  ...
  // 挂载/卸载 body class，屏蔽系统原生光标
  useEffect(() => {
    if (!mounted || isMobile || !hideDefaultCursor) return
    document.body.classList.add(CURSOR_BODY_CLASS)
    return () => {
      document.body.classList.remove(CURSOR_BODY_CLASS)
    }
  }, [mounted, isMobile, hideDefaultCursor])
